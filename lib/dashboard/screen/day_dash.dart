...
import '../../model/Transaction.dart';
 ...
 class DashDay extends StatefulWidget {
   const DashDay({super.key}); 
  
  @override
  State<DashDay> createState() => _DashDayState();
}

class _DashDayState extends State<DashDay> { 
 @override
  void initState() {
     initData() ;
    super.initState();
 ...
 Future<void> initData() async {  
  final  dateNow = DateTime.now() ;  
   final dayhNow = dateNow.day;
   print(dayhNow) ;
 final circularnow = await DashController.getCircularData( dayhNow ,'day' ) ;  
 final chartnowInc = await DashController.getchartdata( 'Income' ,dayhNow ,'day'  ) ;  
 final chartnowEx = await DashController.getchartdata('Expense' , dayhNow ,'day' ) ;  
setState(() {
   _chartdtaaEx = chartnowEx ; 
   _chartdtaaInc =chartnowInc ; 
   _circulardata = circularnow ;
});
         ...
         child: Row( 
          mainAxisAlignment: MainAxisAlignment.center,
           children: [
               ...
               readOnly: true,
                  ...
                  onTap:() async {   
                    ...
                    datecontroller.text =DateFormat.yMMMd().format(pickDate) ;   
                     date = DateFormat.yMMMd().parse(datecontroller.text).day ;
                    }
            ...
            InkWell(
              onTap: () async{
                   //final endate  =DateFormat.yMMMd().add_jm().parse() ;  
                     ...
                     final newdataCircular  = await DashController.getCircularData( date ,'day' ) ;  
                     final newchartdataINC = await DashController.getchartdata( 'Income' , date ,'day'  ) ;  
                     final newchartdataEx = await DashController.getchartdata('Expense' ,date ,'day' ) ; 
            ...
            CircularCard(
              titlechart: ' Daily Incomes & Expenses',
                 data:_circulardata,
              ) , 
            ...
            child: Column(
              children: [
                ...
                (dataSource: _chartdtaaEx, 
                 xValueMapper: (Transactions data, _)=>  DateFormat.Hm().parse(data.time) ,
